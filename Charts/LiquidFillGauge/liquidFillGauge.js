 ...
 * deom: http://bl.ocks.org/brattonc/5e5ce9beee483220e2f6
 */
import $ from 'jquery';
...
function loadLiquidFillGauge(elementId, value, config) {
    ...
    var textFinalValue = parseFloat(value).toFixed(3);
    ...
    if(parseFloat(textFinalValue) != parseFloat(textRounder(textFinalValue))){
        textRounder = function(value){ return parseFloat(value).toFixed(3); };
    }
    ...
    var gaugeGroup = gauge.append("g")
          .attr('transform','translate(0,'+locationY+')');
    ...
    function GaugeUpdater(){
        this.update = function(value, color, chartId){
            ...
            var textTween = function(){
                ...
                const _this = this;
                return function(t) {
                  _this.textContent = textRounderUpdater(i(t)) + percentText;
                }
            };
            ...
            text2.transition()
                .duration(config.waveRiseTime)
                .tween("text", textTween);
            $(`#${chartId}`).find('path, circle').css({fill: color}) // 修改水位图的颜色
