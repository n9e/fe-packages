...
import { Button, message, Form, Input, Icon, Card, Popover, Popconfirm } from "antd";
        ...
        props.form.validateFields(async (err, values) => {
            if (!err) {
                ...
                try {
                    ...
                    }, false).then(() => message.success('修改密码成功,请重新登录！'));
    ...
    return (
        <div className={prefixCls}>
            <div className={`${prefixCls}-top`}>
                ...
                />
                <span className={`${prefixCls}-top-text`}>
                    {footer.title}
                </span>
                <a
            ...
            <Card className={`${prefixCls}-content`} title={<span style={{fontSize: 16}}>修改密码 | <span style={{ fontSize: 12 }}>{query.reason}</span></span>}>
                <Form layout="vertical" onSubmit={handleSubmit} style={{width: 450, margin: 'auto'}}>
            ...
            <div className={`${prefixCls}-bottom`}>
                <div className={`${prefixCls}-bottom-text`}>
                    ...
                    <span>{footer.loginFooter?.contactInfo[2]}{total}次</span>
                </div>
            </div>
        </div>
    );
